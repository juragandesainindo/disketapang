  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="color:black;">
      <div class="modal-header bg-primary">
        <strong style="font-size: 18px;">Tambah Daftar Bantuan</strong>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ route('daftar-bantuan-kamapan.store') }}" method="POST" enctype="multipart/form-data">
      	@csrf
	      <div class="modal-body" style="overflow-y: auto;">
	          <input type="hidden" name="kamapan_daftar_id" value="{{ $data->id }}">
...
@switch($edit)
@case(1)
@foreach($data->kamapanDaftarBantuan as $item)
<div class="modal fade" id="edit-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content" style="color:black;">
      <div class="modal-header bg-primary">
        <strong style="font-size: 18px;">Edit Daftar Bantuan</strong>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ route('daftar-bantuan-kamapan.update',$item->id) }}" method="POST" enctype="multipart/form-data">
      	@csrf
      	@method('PUT')
	      <div class="modal-body" style="overflow-y: auto;">
		        <div class="form-group">
		    ...
		    <div class="col-lg-12 p-0">
                <div class="page-header">
                    <div class="page-title" style="display: flex;justify-content: space-between;align-items: center;">
                        <label class="m-t-10">
                    	    <a href="{{ url('daftar-kamapan') }}" class="btn btn-default">Kembali</a>
                    	    @if(Auth::user()->role_name=='Kadis Ketapang')
                            @else
                    	    <a href="#" class="btn btn-info" data-toggle="modal" data-target="#add">Tambah</a>
                    	    @endif
                    	</label>
                        <h1 class="text-primary" style="text-transform: uppercase;">@yield('title') <span class="badge badge-info">{{ $data->dktKelompok->nama_dkt }}</span></h1>
                    </div>
                </div>
            </div>
		            ...
		            <div class="card-body">
		                <div class="tab-content" style="overflow-x: auto;">
	                        ...
	                        <table class="table table-striped table-hover" id="table1">
	                        	...
	                        	<tbody>
	                        		@foreach($data->kamapanDaftarBantuan as $key => $item)
	                        		<tr>
	                        			<td>{{ ++$key }}</td>
                                        ...
                                        @else
	                        			<td>
	                        				<form action="{{ route('daftar-bantuan-kamapan.destroy',$item->id) }}" method="POST" enctype="multipart/form-data">
