...
<div class="modal fade" id="add" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{ route('kelompok-wanita-tani-kelurahan.store') }}" method="POST" enctype="multipart/form-data">
      	@csrf
	      <div class="modal-body">
	      		<input type="hidden" name="kwt_kecamatan_id" value="{{ $data->id }}">
	...
	<div class="modal fade" id="edit-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	  <div class="modal-dialog" role="document">
	    <div class="modal-content">
	      ...
	      <form action="{{ route('kelompok-wanita-tani-kelurahan.update',$item->id) }}" method="POST" enctype="multipart/form-data">
	      	@csrf
	      	@method('PUT')
		      <div class="modal-body">
			        <div class="form-group">
			        	<div class="row">
			        		...
			        		<div class="col-lg-9">
			        			<input type="text" name="kelurahan" class="form-control" value="{{ $item->kelurahan }}" required>
			        		</div>
			        	</div>
			        </div>
		      </div>
                ...
                <div class="page-header">
                    <div class="page-title" style="display: flex;justify-content: space-between;align-items: center;">
                    	<label style="margin-top: 10px;">
                    		<a href="{{ route('kelompok-wanita-tani.index') }}" class="btn btn-default btn-outline">Kembali</a>
                        ...
                        <h1 class="text-primary" style="text-transform: uppercase;">@yield('title') - Kecamatan {{ $data->kecamatan }}</h1>
                    </div>
                </div>
            </div>
		            ...
		            <div class="card-body">
		                <div class="tab-content">
	                        ...
	                        <table class="table table-striped table-hover" id="table1">
	                        	...
	                        	<tbody>
	                        		@foreach($data->kelurahan as $key => $item)
	                        		<tr>
	                        			<td>{{ ++$key }}</td>
	                        			<td>{{ $item->kelurahan }}</td>
	                        			<td>{{ $item->kelompok->count() }}</td>
	                        			<td>
	                        				<form action="{{ route('kelompok-wanita-tani-kelurahan.destroy',$item->id) }}" method="POST" enctype="multipart/form-data">
	                        					<a href="{{ route('kelompok-wanita-tani-kelurahan.show',$item->id) }}" class="btn btn-info btn-sm" target="_blank"><i class="ti-eye"></i></a>
	                        					@if(Auth::user()->role_name=='Kadis Ketapang')
	                        					@else
