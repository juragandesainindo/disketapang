  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <strong style="font-size: 18px;">Tambah Bantuan Berita Acara</strong>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ route('kwt-berita-acara-bantuan.store') }}" method="POST" enctype="multipart/form-data">
      	@csrf
	      <div class="modal-body">
	          <input type="hidden" name="kwt_berita_id" value="{{ $data->id }}">
...
@switch($edit)
@case(1)
@foreach($data->kwtBeritaBantuan as $item)
<div class="modal fade" id="edit-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <strong style="font-size: 18px;">Edit Bantuan Berita Acara</strong>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="{{ route('kwt-berita-acara-bantuan.update',$item->id) }}" method="POST" enctype="multipart/form-data">
      	@csrf
      	@method('PUT')
	      <div class="modal-body">
		        <div class="form-group">
                ...
                <div class="page-header">
                    <div class="page-title" style="display: flex;justify-content: space-between;align-items: center;">
                        <label style="margin-top:10px;">
                        	<a href="{{ url('kwt-berita-acara') }}" class="btn btn-default btn-outline">Kembali</a>
                        	@if(Auth::user()->role_name=='Kadis Ketapang')
                        	@else
                        	<a href="#" class="btn btn-info" data-toggle="modal" data-target="#add">Tambah</a>
                        	@endif
                    	</label>
                        <h1 class="text-primary" style="text-transform: uppercase;">@yield('title') <span class="badge badge-primary">{{ $data->kwtKelompok->nama_kwt }}</span></h1>
                    </div>
                </div>
            </div>
		            ...
		            <div class="card-body">
		                <div class="tab-content">
	                        ...
	                        <table class="table table-striped table-hover" id="table1">
	                        	...
	                        	<tbody>
	                        		@foreach($data->kwtBeritaBantuan as $key => $item)
	                        		<tr>
	                        		    <td>{{ ++$key }}</td>
	                        		    ...
	                        		    @else
	                        			<td>
	                        				<form action="{{ route('kwt-berita-acara-bantuan.destroy',$item->id) }}" method="POST" enctype="multipart/form-data">
