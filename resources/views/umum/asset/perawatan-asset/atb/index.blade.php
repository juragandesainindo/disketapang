...
@section('title','Perawatan Asset Tak Berwujud')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{ route('perawatan-asset-tak-berwujud.index') }}" method="GET">
    ...
    <div class="container-fluid p-0">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div>
                ...
                <a href="{{ route('perawatan-asset-tak-berwujud.create') }}" class="btn btn-primary">
                ...
                <div class="card flex-fill px-3 pb-3 pt-3">
                    ...
                    <table id="example" class="table table-striped" style="width:100%;">
                        <thead>
                            <tr>
                                <th>Kode</th>
                                <th>Barang&nbsp;/&nbsp;Instansi&nbsp;/&nbsp;Developer</th>
                                <th>Tgl</th>
                                <th>Uraian</th>
                                <th>Nilai (Rp)</th>
                                <th>Keterangan</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $item->assetUmum->mappingAsset->nama_brg }} <br>
                                    ( {{ $item->assetUmum->instansi_developer }} / {{ $item->assetUmum->nama_developer
                                    }} )
                                </td>
                                <td>{{ $item->tgl }}</td>
                                <td>{{ $item->uraian }}</td>
                                ...
                                <td>
                                    ...
                                    @else
                                    <a href="{{ asset('umum/asset/perawatan/atb/'.$item->foto) }}" target="_blank">
                                        <img src="{{ asset('umum/asset/perawatan/atb/'.$item->foto) }}" width="36"
                                ...
                                <td>
                                    <a class="btn btn-info btn-sm mb-1" title="Expot PDF" target="_blank"
                                        href="{{ route('perawatan-asset-tak-berwujud.show',$item->id) }}">
                                        <i data-feather="download-cloud"></i> Export&nbsp;PDF
                                    </a>
                                    <a class="btn btn-warning btn-sm mb-1"
                                        href="{{ route('perawatan-asset-tak-berwujud.edit',$item->id) }}">
...
<div class="modal fade" id="delete-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('perawatan-asset-tak-berwujud.destroy',$item->id) }}" method="POST"
                ...
                <div class="modal-body">
                    <p>
                        ...
                        Instansi : <strong>{{ $item->assetUmum->instansi_developer }}</strong> <br>
                        Developer : <strong>{{ $item->assetUmum->nama_developer }}</strong> <br>
