...
@section('title','Perawatan Asset Kib E')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body">
                <form action="{{ route('perawatan-asset-kib-e.index') }}" method="GET">
    ...
    <div class="container-fluid p-0">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div>
                ...
                <a href="{{ route('perawatan-asset-kib-e.create') }}" class="btn btn-primary">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $item->assetUmum->mappingAsset->nama_brg }} <br>
                                    ( {{ $item->assetUmum->judul }} / {{ $item->assetUmum->penerbit }} )
                                </td>
                                <td>{{ $item->tgl }}</td>
                                <td>{{ $item->uraian }}</td>
                                ...
                                <td>
                                    ...
                                    @else
                                    <a href="{{ asset('umum/asset/perawatan/kib-e/'.$item->foto) }}" target="_blank">
                                        <img src="{{ asset('umum/asset/perawatan/kib-e/'.$item->foto) }}" width="36"
                                ...
                                <td>
                                    <a class="btn btn-info btn-sm mb-1" title="Expot PDF" target="_blank"
                                        href="{{ route('perawatan-asset-kib-e.show',$item->id) }}">
                                        <i data-feather="download-cloud"></i> Export&nbsp;PDF
                                    </a>
                                    <a class="btn btn-warning btn-sm mb-1"
                                        href="{{ route('perawatan-asset-kib-e.edit',$item->id) }}">
...
<div class="modal fade" id="delete-{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('perawatan-asset-kib-e.destroy',$item->id) }}" method="POST"
                ...
                <div class="modal-body">
                    <p>
                        ...
                        Judul : <strong>{{ $item->judul }}</strong> <br>
                        Penerbit : <strong>{{ $item->assetUmum->penerbit }}</strong> <br>
                        Keterangan : <strong>{{ $item->assetUmum->keterangan }}</strong> <br>
