    ...
    <div class="col-12 col-lg-12">
        <div class="card flex-fill px-2 pb-2">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="card-title text-dark mb-0">Riwayat Pendidikan dan Pelatihan Kepemimpinan</h5>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                    data-bs-target="#createPelatihanKepemimpinan">
                    <i data-feather="folder-plus"></i>&nbsp;
                    Add Pelatihan Kepemimpinan
                </button>
            </div>
            <div class="card-body">
                <table id="pelatihanKepemimpinan" class="table table-striped" style="width:100%;">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama Diklat</th>
                    ...
                    <tbody>
                        @foreach ($pegawai->pelatihanKepemimpinan->sortByDesc('tahun') as $key => $item)
                        <tr>
                            <td>{{ ++$key }}</td>
                            ...
                            <td>
                                <a href="#" data-bs-toggle="modal"
                                    data-bs-target="#editPelatihanKepemimpinan-{{ $item->id }}"
                                    class="btn btn-info btn-sm mb-1"><i data-feather="edit"></i></a>
                                <a href="#" data-bs-toggle="modal"
                                    data-bs-target="#deletePelatihanKepemimpinan-{{ $item->id }}"
...
<div class="modal fade" id="createPelatihanKepemimpinan" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myLargeModalLabel">Create Pelatihan Kepemimpinan</h4>
...
<div class="modal fade" id="editPelatihanKepemimpinan-{{ $item->id }}" tabindex="-1" role="dialog"
    aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myLargeModalLabel">Edit Pelatihan Kepemimpinan</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('pegawai-pelatihan-kepemimpinan.update',$item->id) }}" method="POST"
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <embed src="{{ asset('umum/pegawai/pelatihan-kepemimpinan/'.$item->foto) }}" width="100%">
                    </div>
                    <input type="hidden" name="pegawai_id" value="{{ $pegawai->id }}">
                </div>
                <div class="modal-footer">
...
@foreach ($pegawai->pelatihanKepemimpinan as $item)
<div class="modal fade" id="deletePelatihanKepemimpinan-{{ $item->id }}" tabindex="-1" role="dialog"
    aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myLargeModalLabel">Hapus Pelatihan Kepemimpinan</h4>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('pegawai-pelatihan-kepemimpinan.destroy',$item->id) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('DELETE')
                <div class="modal-body">
